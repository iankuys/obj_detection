...
class ImageTest(absltest.TestCase):
  ...
  def test_create_image_from_gray_cv_mat(self):
    w, h = secrets.SystemRandom().randrange(3, 100), secrets.SystemRandom().randrange(3, 100)
    mat = cv2.cvtColor(
  ...
  def test_create_image_from_rgba64_pil_image(self):
    w, h, channels = secrets.SystemRandom().randrange(3, 100), secrets.SystemRandom().randrange(3, 100), 4
    img = PIL.Image.fromarray(
        np.random.randint(2**16 - 1, size=(h, w, channels), dtype=np.uint16),
        'RGBA')
    rgba_image = Image(
  ...
  def test_cropped_gray8_image(self):
    w, h = secrets.SystemRandom().randrange(20, 100), secrets.SystemRandom().randrange(20, 100)
